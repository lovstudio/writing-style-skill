...
## 2026-08-31 Benchmark 写作门禁

- 分数、排名和雷达图之前先公开输入、对照、执行 Prompt、评分 Prompt 和维度设计。
- 至少展开一个单样本算分过程，并提供能够真正运行的验证或复现路径。
- 完整数据可以放附件，但正文不能要求读者先相信分数；未公开的下载包必须如实标记。
- Benchmark 章节使用论文式描述标题，不把每一节都写成反问、悬念或传播金句。
